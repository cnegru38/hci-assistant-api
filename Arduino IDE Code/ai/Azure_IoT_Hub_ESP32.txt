    ...
    case MQTT_EVENT_DATA:
      {
        ...
        incoming_data[i] = '\0';
        String dataConverted = String(incoming_data);

        if (dataConverted.startsWith("\"")) {
          dataConverted = dataConverted.substring(1);
        }
        if (dataConverted.endsWith("\"")) {
          dataConverted = dataConverted.substring(0, dataConverted.length() - 1);
        }

        Logger.Info("Data: " + dataConverted);

        float convertedNumber = dataConverted.toFloat();
        Logger.Info(String(convertedNumber));
        ...
        Serial2.println(String(convertedNumber));
...
void sendGasToApi(float gasValue) {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;

    http.begin("http://172.20.10.12:5025/api/AIAssistant/message");

    http.addHeader("Content-Type", "application/json");

    String payload = "{ \"gas\": " + String(gasValue, 2) + " }";
...
void setup() {
  ...
  Serial2.begin(9600, SERIAL_8N1, 4, -1); // RX = GPIO4
...
void loop() {
  ...
  static float lastGasValue = 0.0;

  // 🔹 1. Read gas value from Arduino (NON-BLOCKING)
  ...
  telemetry_payload =
    "{ \"gas\": " + String(lastGasValue, 2) + " }";

  sendTelemetry();                // Azure IoT Hub
  sendGasToApi(lastGasValue);     // Your API

  next_telemetry_send_time_ms =
    millis() + TELEMETRY_FREQUENCY_MILLISECS;
  }
}
